...
export const zhHans: TranslationStructure = {
    ...
    machine: {
        systemHealth: {
            ...
            empty: {
                unsupported: '更新远端 Paws CLI 后可使用系统监控。',
                disabled: '这台设备尚未启用系统监控。',
                pending: '正在等待守护进程初始化系统监控。',
                collecting: '正在采集第一份系统状态。',
                unavailable: '暂时无法获取系统监控数据。',
            },
            metrics: { cpu: 'CPU', memory: '可用内存', swap: 'Swap', processes: '进程', zombies: '僵尸进程', orphans: '孤儿 Worker' },
            facts: {
                zombies: ({ count }: { count: number }) => `僵尸进程：${count}`,
                load: ({ one, five, fifteen }: { one: string; five: string; fifteen: string }) => `负载 1/5/15 分钟：${one} / ${five} / ${fifteen}`,
                pressure: ({ percent }: { percent: string }) => `内存压力可用：${percent}%`,
                compressed: ({ value }: { value: string }) => `压缩内存：${value}`,
                disk: ({ value }: { value: string }) => `磁盘可用：${value}`,
                workers: ({ roots, processes, rss }: { roots: number; processes: number; rss: string }) => `Paws Worker：${roots} 个根 · ${processes} 个进程 · ${rss}`,
                orphans: ({ roots, processes, rss }: { roots: number; processes: number; rss: string }) => `孤儿 Worker：${roots} 个根 · ${processes} 个进程 · ${rss}`,
            },
            issues: {
                'orphan-workers': '检测到孤儿 Worker', 'swap-high': 'Swap 使用量过高', 'swap-growing': 'Swap 增长过快',
                'cpu-sustained': 'CPU 持续高占用', 'load-high': '系统负载过高', 'memory-pressure-high': '内存压力过高',
                'worker-memory-high': 'Worker 内存占用过高', 'process-count-high': '进程数量过多',
                'process-capacity-high': '进程容量即将耗尽', 'zombie-processes': '僵尸进程持续存在',
                'disk-low': '磁盘空间不足', 'single-source-cpu-high': '单一来源持续占用大量 CPU',
            },
            statusSummary: ({ status }: { status: string }) => `系统稳定性：${status}`,
            updatedAgo: ({ seconds }: { seconds: number }) => `${seconds} 秒前更新`,
            collectorErrors: ({ count }: { count: number }) => `${count} 个采集错误`,
            chartSummary: ({ label, min, max, latest }: { label: string; min: string; max: string; latest: string }) => `${label}：最低 ${min}，最高 ${max}，当前 ${latest}`,
            collectingTrend: '正在收集趋势数据',
            range: ({ min, max }: { min: string; max: string }) => `最低 ${min} · 最高 ${max}`,
            trends: '最近 30 分钟', sources: '主要资源来源', zombieSources: '僵尸进程来源',
            zombieShort: ({ count }: { count: number }) => `${count} 个僵尸`,
        },
