...
export const ja: TranslationStructure = {
    delivery: {
        tunnelMissing: "一時トンネルを開くには、実行マシンに cloudflared をインストールする必要があります。",
        hostingUnconfigured: "クラウドホスティングが未設定です。先に Cloudflare を設定してください。",
        published: "前回の公開に成功",
        saved: "共有設定を保存しました · 次のメッセージから適用",
        updatedElsewhere: "別のデバイスで共有設定が更新されました",
        title: "成果の共有設定",
        away: "パソコンから離れている",
        description: "スマートフォンで開けるリンクや添付ファイルを優先",
        scope: "すべてのセッションに適用し、デバイス間で同期",
        timing: "次のメッセージから適用。実行中のタスクには影響しません",
        previewMode: "Web プレビュー方式",
        tunnel: "一時トンネル",
        tunnelDescription: "Cloudflare アカウントは不要です。実行マシンとセッションの稼働が必要です。最長24時間。",
        hosted: "クラウドホスティング",
        hostedDescription: "静的ページをアップロードし、24時間プレビューできます。実行マシンの停止後も閲覧できます。",
        configure: "Cloudflare を設定",
        unconfigured: "未設定",
        unverified: "設定済み · 検証待ち",
        verified: "前回の確認に成功",
        authorizationError: "認証の問題 · トークンと権限を確認",
        verificationUnavailable: "現在は検証できません",
        check: "接続を確認",
        checking: "接続を確認中…",
        saveAndCheck: "保存して確認",
        tokenConnection: "Account ID と API Token を使用します。保存済みのトークンは表示されません。",
        enabledLabel: "パソコンから離れている：オン",
        disabledLabel: "パソコンから離れている：オフ",
        publicationFailed: "公開に失敗しました。再試行するか設定を確認してください",
        publicationAuthError: "Cloudflare の認証に失敗しました。設定を更新してください。",
        retryPublication: "再公開をリクエスト",
        useTunnelOnce: "今回のみ一時トンネルを使用",
        retryRequest: "このプレビューを元のプレビュー方式で再公開してください。",
        tunnelOnceRequest: "今回のみ一時トンネルでこのプレビューを再公開してください。全体の設定は変更しないでください。",
        checkedAt: ({ time }: { time: string }) => `確認日時：${time}`,
    },
    accounts: {
    ...
    machine: {
        ...
        codexUsageAllAccounts: 'すべてのアカウント',
        codexUsageUnattributed: '未割り当て',
        codexUsageSelectAccount: 'アカウントを選択して残りのクォータを確認してください。',
        codexUsageUnattributedHint: 'Paws アカウントが明示されていないローカル Codex セッションは未割り当てのままです。',
        codexUsageToday: '今日',
