...
export const agentKeys = ['claude', 'codex', 'gemini', 'opencode', 'openclaw', 'deepseek'] as const;
...
export const AgentDefaultOverridesSchema = z.object({
    ...
    deepseek: AgentDefaultOverrideSchema.optional(),
}).passthrough().default({});
...
const codeAgentDefaults: Record<AgentKey, AgentDefaultConfig> = {
    ...
    deepseek: { permissionMode: 'default', modelMode: 'deepseek-v4-flash', effortLevel: null },
};

export function normalizeAgentKey(flavor: string | null | undefined): AgentKey {
    if (flavor === 'codex' || flavor === 'gemini' || flavor === 'opencode' || flavor === 'openclaw' || flavor === 'deepseek') {
        return flavor;
    }
    return 'claude';
}
