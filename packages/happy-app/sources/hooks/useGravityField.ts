 ...
 *
 * 两条通道，两种手感：
 *  1) 倾斜飘移（gx / gy）：用 GRAVITY 传感器（已被系统低通成纯重力向量）。竖握时
 *     重力恒指向屏幕下方，会让粒子一直往一边堆——所以这里**追踪「中性握姿」基线**
 *     （neutralX/Y 是重力的慢速长平均），只对「偏离基线的倾斜量」响应：倾斜手机时
 *     整团朝倾斜方向晃出去，保持新角度则基线缓慢追上、粒子慢慢回中。像水平仪气泡。
...
export interface GravityField {
    gx: SharedValue<number>;     // 归一化倾斜偏移 X
...
export function useGravityField(): GravityField {
    ...
    useAnimatedReaction(
        ...
        (g) => {
            ...
            gx.value += (tx - gx.value) * SMOOTH;
            gy.value += (ty - gy.value) * SMOOTH;
        },
    );
