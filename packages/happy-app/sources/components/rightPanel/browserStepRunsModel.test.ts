...
describe('getBrowserStepRuns', () => {
    ...
    it('isolates child agent queues and displays explicit orphan browser runs independently', () => {
        ...
        expect(visible[1].children).toHaveLength(1);
        expect(visible[1].children[0].tool).toMatchObject({ name: 'ego-browser', input: { runId: 'run-b' } });
        expect(visible[2]).toBe(ordinary);
        expect(childA.children).toHaveLength(2);
    });
    it('keeps explicit runs standalone rather than guessing ownership among concurrent invocations', () => {
        ...
        expect(runs.map((run) => ({
            ...
            { id: 'producer-a', invocationMessageId: 'a-1', stepIds: ['a-1', 'a-2'] },
            { id: 'producer-b', invocationMessageId: 'b-1', stepIds: ['b-1'] },
    ...
    it('binds the producer run ID to the latest matching real Skill invocation exactly once', () => {
        ...
        expect(runs.map((run) => ({
            ...
            { id: 'generated-browser-2', invocationMessageId: 'orphan-second-id', stepIds: ['orphan-second-id', 'browser-3'] },
