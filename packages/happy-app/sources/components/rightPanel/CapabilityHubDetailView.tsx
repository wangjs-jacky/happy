...
export const SkillItemRow = React.memo(function SkillItemRow(props: { title: string; sessionId: string }) {
    const { theme } = useUnistyles();
    return (
        <View style={[styles.rowCard, { backgroundColor: theme.colors.surface, borderColor: theme.colors.divider }]}>
            <View style={[styles.rowIconWrap, { backgroundColor: theme.colors.surfaceHigh }]}>
                <Ionicons color={theme.colors.textSecondary} name="flash-outline" size={15} />
            </View>
            <View style={styles.rowCopy}>
                <Text numberOfLines={1} style={[styles.rowTitle, { color: theme.colors.text }]}>{props.title}</Text>
                <Text numberOfLines={1} style={[styles.rowMeta, { color: theme.colors.textSecondary }]}>
