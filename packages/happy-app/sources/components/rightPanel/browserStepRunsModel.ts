 ...
 * a unique pending matching invocation, unless an invocation/call alias identifies
...
export function getBrowserStepRuns(messages: Message[]): BrowserStepRun[] {
    ...
    for (const message of orderedMessages) {
        ...
        if (step.runId && !run && step.skillName) {
            ...
            const pending = skillName ? pendingInvocationsBySkill.get(skillName) ?? [] : [];
            // Never guess which concurrent invocation owns an explicit run.
            const candidate = pending.length === 1 ? pending[0] : undefined;
            if (candidate) {
                bindRunId(candidate, step.runId);
                run = candidate;
            }
        }
        if (!run && step.runId && asEgoSkillName(step.skillName)) {
            // Register an explicit orphan immediately: later frames keep this
            // same owner even if a Skill event arrives between two captures.
            run = {
                id: step.runId, invocationMessageId: step.id, createdAt: step.createdAt,
                skillName: step.skillName as EgoSkillName, steps: [],
                aliases: new Set([step.runId]), boundExplicitRunId: step.runId,
            };
            runs.push(run);
            addAlias(step.runId, run);
        }
        if (!run) continue;
...
/** Keep every run reachable in Skills, including Codex without a Skill tool event. */
export function hideLinkedBrowserSteps(messages: Message[], runs: BrowserStepRun[]): Message[] {
    ...
    const standalone = new Map(runs.filter(run => run.invocationMessageId === run.steps[0]?.id)
        .map(run => [run.invocationMessageId, run]));
    const filter = (items: Message[]): Message[] => items.flatMap(message => {
        const run = standalone.get(message.id);
        if (run && message.kind === 'tool-call') {
            // A lightweight Skill row replaces the transport file, not an image bubble.
            return [{ ...message, tool: { ...message.tool, name: run.skillName,
                input: { runId: run.id }, result: undefined }, children: [] }];
        }
