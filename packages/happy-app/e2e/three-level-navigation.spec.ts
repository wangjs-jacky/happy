...
async function dismissSessionActionsMenu(page: Page): Promise<void> {
    const pinAction = page.getByText('Pin Session', { exact: true });
    await page.keyboard.press('Escape');
    if (await pinAction.isVisible()) {
        // React Native Web's modal backdrop owns the pointer while the menu is
        // open. A direct pointer click models a user dismissing that backdrop
        // without asking Playwright to click through it to covered content.
        await page.mouse.click(1100, 700);
    }
    await expect(pinAction).toHaveCount(0);
}
...
test('[THREE-LEVEL-NAV-PC] icon rail, organization, sessions, chat, and Capability Hub coexist', async ({ page, request }, testInfo) => {
    ...
    try {
        ...
        await dismissSessionActionsMenu(page);
        ...
        await activateGinghamDarkTheme(page);
        await page.getByTestId(`session-row-${sessionId}`).click();
        await expect(page.locator('[data-testid="session-header-title"]:visible')).toHaveText('Three level desktop conversation');
...
test('[DESKTOP-NAV-MOBILE-ISOLATION] phone Web keeps the established full-text drawer and tab navigation', async ({ page }) => {
    page.setDefaultTimeout(120_000);
    page.setDefaultNavigationTimeout(180_000);
    ...
    await expect(page.getByTestId('desktop-sidebar-tab-projects')).toBeVisible();
    await expect(page.getByTestId('desktop-sidebar-tab-lists')).toBeVisible();
    await expect(page.getByTestId('desktop-sidebar-tab-timeline')).toBeVisible();
