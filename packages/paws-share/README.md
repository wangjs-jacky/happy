...
The HTML uses the same standardized snapshot preparation as public sharing. CSS, interaction code, images, audio, video, and downloadable file attachments are embedded in the output, so it opens offline. Existing files are preserved unless `--force` is explicit. High-confidence secrets and unresolved attachments remain blocked by default. Transcript-recorded working directories are resolution hints only; attachment access stays limited to the session directory, the invoking project, and Happy's configured attachment staging directory.
