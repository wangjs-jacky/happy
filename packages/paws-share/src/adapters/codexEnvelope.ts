...
const GENERATED_ATTACHMENT_NOTICE = /^Happy attached \d+ user-uploaded images? to this Codex turn\.[\s\S]*?(?=<!-- happy:(?:paws-origin:|system-prompt:start))/i;
...
function isSyntheticConfigurationText(value: string): boolean {
...
export function isSyntheticPawsCodexMessage(content: unknown[]): boolean {
    return content.length > 0 && content.every((block) => {
        ...
        return text !== null && isSyntheticConfigurationText(text);
    });
}
