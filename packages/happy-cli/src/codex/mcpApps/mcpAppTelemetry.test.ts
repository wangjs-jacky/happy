...
describe('MCP App CLI telemetry', () => {
    it('serializes only bounded allowlisted fields when callers carry canary secrets', () => {
        ...
        expect(payload).toEqual({
            ...
            originScoped: true,
            outcomeCode: 'MCP_APP_PERMISSION_DENIED',
        });
    ...
    it('keeps sink failures inert', () => {
        ...
        expect(() => emitMcpAppTelemetry('mcp_app_tool_call_requested', {
            platform: 'cli',
            stage: 'tool_call',
            durationMs: 0,
            byteLength: 0,
    ...
    it('attaches an asynchronous rejection handler without awaiting the sink', async () => {
        ...
        emitMcpAppTelemetry('mcp_app_tool_call_requested', {
            platform: 'cli',
            stage: 'tool_call',
            durationMs: 0,
            byteLength: 0,
