...
describe('interactive preview integration', () => {
    ...
    it('runs authenticated draft/upload/complete/publish through routes, emits a typed ready event, and removes staging', async () => {
        ...
        const baseUrl = `/v1/sessions/s-a/previews/${previewId}`;
        const draft = await typed.inject({ method: 'POST', url: `${baseUrl}/draft`, headers: { 'x-user-id': 'a' }, payload: manifest }); expect(draft.statusCode).toBe(200);
        objects.set(rows.get(previewId).assets[0].storageKey, bytes); // direct upload at the exact persisted private S3 key
        expect((await typed.inject({ method: 'POST', url: `${baseUrl}/assets/index/uploaded`, headers: { 'x-user-id': 'a' } })).statusCode).toBe(200);
        const published = await typed.inject({ method: 'POST', url: `${baseUrl}/publish`, headers: { 'x-user-id': 'a' } }); expect(published.statusCode, published.body).toBe(200); expect(published.json().preview).toMatchObject({ version: 1, id: previewId, state: 'ready', url: 'https://preview.local' }); expect(objects.size).toBe(0); expect(uploads).toHaveLength(2);
        expect((await typed.inject({ method: 'GET', url: '/v1/sessions/s-a/previews', headers: { 'x-user-id': 'b' } })).statusCode).toBe(404);
        expect((await typed.inject({ method: 'POST', url: `${baseUrl}/assets/index/uploaded`, headers: { 'x-user-id': 'b' } })).statusCode).not.toBe(200);
        expect((await typed.inject({ method: 'POST', url: `${baseUrl}/publish`, headers: { 'x-user-id': 'b' } })).statusCode).not.toBe(200);
        await typed.inject({ method: 'DELETE', url: baseUrl, headers: { 'x-user-id': 'b' } }); expect(rows.get(previewId).status).toBe('ready');
    ...
    it('holds a global two-publication cap, keeps per-preview uploads sequential, and returns an in-flight duplicate', async () => {
        ...
        const make = (id: string) => { const body = Buffer.from(id); const sha = createHash('sha256').update(body).digest('hex'); const stagingGeneration = `generation-${id.slice(0, 8)}`; const row: any = { id, accountId: 'a', sessionId: 's-a', stagingGeneration, title: id, status: 'draft', url: null, publishedAt: null, expiresAt: new Date(Date.now() + 60_000), errorCode: null, cleanupClaimedAt: null, assets: [{ id: 'one', path: 'index.html', mimeType: 'text/html', size: body.length, sha256: sha, storageKey: storage.storageKey({ accountId: 'a', previewId: id, stagingGeneration }, 'one'), uploadedAt: new Date() }, { id: 'two', path: 'app.js', mimeType: 'text/javascript', size: body.length, sha256: sha, storageKey: storage.storageKey({ accountId: 'a', previewId: id, stagingGeneration }, 'two'), uploadedAt: new Date() }] }; rows.set(id, row); objects.set(row.assets[0].storageKey, body); objects.set(row.assets[1].storageKey, body); return row; };
        ...
        const first = service.publish('a', 's-a', ids[0]); const second = service.publish('a', 's-a', ids[1]); const third = service.publish('a', 's-a', ids[2]); await new Promise((resolve) => setTimeout(resolve, 0));
        await expect(service.publish('a', 's-a', ids[0])).resolves.toMatchObject({ state: 'publishing' }); expect(deployments).toBe(2); expect(maximum).toBe(2);
    ...
    it('replays persisted stale/deleting tombstones without creating a deployment and honors retry deadlines', async () => {
        const { database, rows } = store(); const { storage } = s3(); const stale: any = { id: '55555555-5555-4555-8555-555555555555', accountId: 'a', sessionId: 's-a', stagingGeneration: 'generation-1', status: 'publishing', vercelDeploymentId: 'dpl_old', expiresAt: new Date(0), updatedAt: new Date(0), cleanupClaimedAt: null, cleanupRetryCount: 0, cleanupNextAttemptAt: null }; rows.set(stale.id, stale);
        ...
        const deferred: any = { id: '66666666-6666-4666-8666-666666666666', accountId: 'a', sessionId: 's-a', stagingGeneration: 'generation-1', status: 'deleting', vercelDeploymentId: 'dpl_later', expiresAt: new Date(0), updatedAt: new Date(0), cleanupClaimedAt: null, cleanupRetryCount: 1, cleanupNextAttemptAt: new Date('2026-09-04T02:00:00Z') }; rows.set(deferred.id, deferred);
    ...
    it('retains an explicit delete tombstone through provider failure then expires it after the durable retry deadline', async () => {
        const { database, rows } = store(); const { storage, objects } = s3(); const id = '77777777-7777-4777-8777-777777777777'; const stagingGeneration = 'generation-1'; rows.set(id, { id, accountId: 'a', sessionId: 's-a', stagingGeneration, title: 'delete', status: 'ready', url: 'https://preview.local', vercelDeploymentId: 'dpl_delete', expiresAt: new Date(0), cleanupClaimedAt: null, cleanupRetryCount: 0, cleanupNextAttemptAt: null }); objects.set(storage.storageKey({ accountId: 'a', previewId: id, stagingGeneration }, 'asset'), Buffer.from('x'));
