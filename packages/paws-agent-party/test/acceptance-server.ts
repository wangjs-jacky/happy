...
await writeFile(index, (await readFile(index, 'utf8')).replace('<body>', '<body><div style="position:fixed;inset:0 0 auto;z-index:100;padding:4px;text-align:center;background:var(--warning);color:var(--background)">测试替身，非真实 Agent</div><style>body{padding-top:28px}#root>div{height:calc(100dvh - 28px)}</style>'));
const dataDir = resolve(process.env.PAWS_AGENT_PARTY_DATA_DIR ?? join(packageDir, '.data-test'));
const server = await createPocServer({ dataDir, staticDir, sdk: new TestOnlySdk({ delayMs: 350 }), port: 0 });
