...
The container is near its 2 GiB memory limit and has a 256-task limit with recorded task-limit rejections. This supports a resource-pressure hypothesis, but the exact early-exit cause is not yet proven: isolated, no-session Codex version checks succeeded and no OOM kill was recorded. Container resource changes need confirmation because the executor also hosts existing work. No limits, timeouts, daemon configuration or existing sessions were changed during diagnosis.
