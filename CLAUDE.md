...
- **版本浏览站**（公开，**必须走 website 域名**）：<https://happy-app-ota-jacky.oss-website-cn-hangzhou.aliyuncs.com/>
  列出 preview 频道全部历史版本（commit/时间），每行一个二维码，手机扫码即把本机锁定到该版本。
  ⚠️ 标准域名 `*.oss-cn-hangzhou.aliyuncs.com/index.html` 会被阿里云强制下载（反钓鱼，Content-Disposition 无法覆盖），
  只有静态网站托管的 `*.oss-website-cn-hangzhou.aliyuncs.com` 域名能在线渲染。已开启静态托管（index document=index.html）。
  网站源码 `packages/happy-app/ota-server/site/index.html`，改完用
  `ossutil cp site/index.html oss://happy-app-ota-jacky/index.html --acl public-read -f` 重传（根目录 index.html）。
...
- **机制**：App 用 `Updates.setExtraParamAsync('ota-target-stamp', <stamp>)` 把目标版本时间戳随
  ...
  `manifests/android/21/preview/<stamp>.json`，取不到静默回退 latest。stamp 纯数字白名单防路径穿越。
- **依赖**：OSS 桶对 `meta/` + `manifests/` 前缀已开匿名 `ListObjects`/`GetObject`（bucket policy，
  用 `oss:Prefix` 条件锁死只能列这两个前缀，不暴露 `updates/` 下 bundle）。改 FC 后需 `cd ota-server && s deploy --use-local -y` 重新部署。
