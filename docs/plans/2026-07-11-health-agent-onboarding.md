...
## Task 7: `HealthWelcomeCard` 组件 + SessionView 空态接入（Phase 2a）

**Files:**
- Create: `packages/happy-app/sources/components/rightPanel/HealthWelcomeCard.tsx`
- Test: `packages/happy-app/sources/components/rightPanel/HealthWelcomeCard.test.tsx`
...
- Produces: `HealthWelcomeCard` 默认导出组件；可见消息数为 0 时渲染。

- [ ] **Step 1: 写渲染测试（先失败）**

`HealthWelcomeCard.test.tsx`：
```typescript
import { describe, it, expect } from 'vitest';
import { render } from '@testing-library/react-native';
import { HealthWelcomeCard } from './HealthWelcomeCard';

describe('HealthWelcomeCard', () => {
    it('renders role and three domains', () => {
        const { getByText } = render(<HealthWelcomeCard />);
        expect(getByText('健康打卡')).toBeTruthy();   // 视测试 locale，默认可能是英文，见 Step 3 说明
...
```
> 若测试环境 locale 为英文，把断言改成 `Health Check-in / Sleep / Exercise / Diet`。先确认仓库现有组件测试用的 locale（参考同目录既有 `*.test.tsx`），与之保持一致。

- [ ] **Step 2: 跑测试确认失败**

Run: `cd packages/happy-app && pnpm test sources/components/rightPanel/HealthWelcomeCard.test.tsx`
Expected: FAIL —— 组件不存在。

- [ ] **Step 3: 实现组件（沿用 SleepHeroCard 的样式语言）**

`HealthWelcomeCard.tsx`：
```typescript
...
```
> 实现前先打开 `SleepHeroCard.tsx` 确认 unistyles 用法（`StyleSheet.create((theme)=>…)` 还是 `useUnistyles()`），与之一致；图标取色改用与其相同的 theme token。
...
// ...
const visibleMessages = messages.filter(m => !m.meta?.hidden);  // 见 Task 9 定义 meta.hidden
if (isHealthCheckinSession(sessionWorkingPath(session)) && visibleMessages.length === 0) {
    return <HealthWelcomeCard />;
}
```
> Task 9 尚未落地时 `m.meta?.hidden` 恒为 undefined，`visibleMessages` 等价于 `messages`，不影响本 Task；两者合并后自动生效。若 `SessionViewLoaded` 当前不在 `SessionView.tsx` 而在独立文件，改对应文件。

- [ ] **Step 6: Commit**

```bash
git add packages/happy-app/sources/components/rightPanel/HealthWelcomeCard.tsx packages/happy-app/sources/components/rightPanel/HealthWelcomeCard.test.tsx packages/happy-app/sources/-session/SessionView.tsx
git commit -m "feat(health): 空健康会话静态欢迎卡"
```
...
**Files:**
- Create: `packages/happy-app/sources/components/rightPanel/HealthDormantState.tsx`
- Test: `packages/happy-app/sources/components/rightPanel/HealthDormantState.test.tsx`
...
- Produces: `HealthDormantState` 组件

- [ ] **Step 1: 写渲染测试（先失败）**

`HealthDormantState.test.tsx`：
```typescript
import { describe, it, expect } from 'vitest';
import { render } from '@testing-library/react-native';
import { HealthDormantState } from './HealthDormantState';

describe('HealthDormantState', () => {
    it('renders dormant title and hint', () => {
        const { getByText } = render(<HealthDormantState />);
        expect(getByText('休眠中')).toBeTruthy();   // locale 同 Task 7 约定
    });
});
```
...
Run: `cd packages/happy-app && pnpm test sources/components/rightPanel/HealthDormantState.test.tsx`
Expected: FAIL。

- [ ] **Step 3: 实现组件（月亮/zzz 休眠视觉，复用欢迎卡的样式语言）**

`HealthDormantState.tsx`：
```typescript
import * as React from 'react';
import { View, Text } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
...
- [ ] **Step 5: 在 HealthCheckinPanel 替换纯文字空态**

`HealthCheckinPanel.tsx` 现有睡眠域空态（约 219-233 行）：
```typescript
return (
...
```
并加 import：`import { HealthDormantState } from './HealthDormantState';`。运动/饮食域若也有 `noExerciseToday`/`noDietToday` 的裸文字空态，同样替换为 `<HealthDormantState />`（保持三域一致）。

- [ ] **Step 6: 跑健康面板相关既有测试确保没破**

Run: `cd packages/happy-app && pnpm test sources/components/rightPanel/`
Expected: PASS（含既有 HealthCheckinPanel/healthLog 测试）。

- [ ] **Step 7: Commit**

```bash
git add packages/happy-app/sources/components/rightPanel/HealthDormantState.tsx packages/happy-app/sources/components/rightPanel/HealthDormantState.test.tsx packages/happy-app/sources/components/rightPanel/HealthCheckinPanel.tsx
...
- Test: `packages/happy-app/sources/sync/hiddenMessage.test.ts`

**Interfaces:**
- Produces: `MessageMeta.hidden?: boolean`；`sendMessage(sessionId, text, { hidden: true })` 使该消息不在列表渲染。
...
- [ ] **Step 2: 写过滤纯函数 + 测试（先失败）**

新增一个可测的纯函数承载过滤逻辑。`ChatList.tsx` 顶部加：
```typescript
...
```typescript
import { describe, it, expect } from 'vitest';
import { filterVisibleMessages } from '../components/ChatList';
...
Run: `cd packages/happy-app && pnpm test sources/sync/hiddenMessage.test.ts`
Expected: FAIL —— 导出不存在。

- [ ] **Step 4: 在 ChatListInternal 渲染前套用过滤**

`ChatListInternal` 里 `useGroupedMessages(props.messages, ...)` 改为：
```typescript
...
```bash
git add packages/happy-app/sources/sync/typesMessageMeta.ts packages/happy-app/sources/sync/sync.ts packages/happy-app/sources/components/ChatList.tsx packages/happy-app/sources/sync/hiddenMessage.test.ts
...
**Files:**
- Create: `packages/happy-app/sources/-session/useHealthGreeting.ts`（幂等触发 hook）
- Test: `packages/happy-app/sources/-session/useHealthGreeting.test.ts`（纯函数部分）
- Modify: `packages/happy-app/sources/-session/SessionView.tsx`（挂载 hook）

**Interfaces:**
- Consumes: `sessionWorkingPath`、`isHealthCheckinSession`、`sync.sendMessage(sessionId, text, { hidden: true })`（Task 9）、`filterVisibleMessages`
- Produces: `shouldGreet(args)` 纯判断 + `useHealthGreeting(sessionId)` hook（每会话仅发一次）

- [ ] **Step 1: 写纯判断函数测试（先失败）**

`useHealthGreeting.test.ts`：
```typescript
import { describe, it, expect } from 'vitest';
import { shouldGreet } from './useHealthGreeting';
...
Run: `cd packages/happy-app && pnpm test sources/-session/useHealthGreeting.test.ts`
Expected: FAIL。

- [ ] **Step 3: 实现纯函数 + hook**

`useHealthGreeting.ts`：
```typescript
import * as React from 'react';
...
export function useHealthGreeting(sessionId: string) {
    ...
    React.useEffect(() => {
        ...
        sync.sendMessage(
            sessionId,
            t('healthPanel.greetingPrompt'),
            { hidden: true },
        );
    }, [sessionId, messages]);
}
```
> `useSessionMessages` 的准确 import 路径与 `ChatList.tsx` 顶部保持一致（照抄其 import）。
...
```bash
git add packages/happy-app/sources/-session/useHealthGreeting.ts packages/happy-app/sources/-session/useHealthGreeting.test.ts packages/happy-app/sources/-session/SessionView.tsx packages/happy-app/sources/text/
...
Run: `cd packages/happy-app && pnpm test`
