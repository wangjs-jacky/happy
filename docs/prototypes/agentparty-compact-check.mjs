...
export async function checkCompact(page) {
  ...
  assert.equal(await page.evaluate(()=>document.querySelector('.members').inert),true,'Members must be closed initially');
  ...
  assert.ok(await page.evaluate(()=>!document.querySelector('.members').inert),'Members should open');
