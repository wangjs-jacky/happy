...
- [ ] **Step 1: 迁移 `2026-06-25.md`**：`夜间睡眠 7h59m`→`总时长: 7h59m`；`零星小睡 1h36m`→`日间小睡: 1h36m`；删 `总睡眠`；`超过用户 99%` 保留；`评分 89` 不动。
- [ ] **Step 2: 迁移 `2026-07-06.md`**：`深睡: 55min`→`深睡: 0h55m`；`总时长/浅睡/快速眼动/评分/质量/入睡/起床/来源` 已合规保留。
- [ ] **Step 3: 用解析器交叉验证**——把两篇迁移后的 frontmatter 各跑一遍 `parseHealthLog`（可临时加断言到 `healthLog.test.ts` 跑一次后删，或用 node 快验），确认每个时长字段解析非 `null`、`sleepScore` 正确。
