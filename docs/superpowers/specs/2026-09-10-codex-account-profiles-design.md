...
- 账号库显示最近一次由 Paws Codex 会话观测到的 7d 剩余额度和重置时间，不为刷新额度额外启动 Codex 请求。
...
10. 账号卡片显示 `7d 剩余百分比 + 重置时间`；额度仅随 Paws 发起的 Codex 会话被动更新，不主动轮询或启动隐藏请求。
...
- 每个账号最近一次 7d 额度快照、重置时间、观测时间和过期状态。
- 旧版单一 OpenAI 凭据的兼容迁移。
...
### 5.2 CodexQuotaSnapshot

每个 profile 最多保存一份最近额度快照。它是运行观测值，不是账单或实时查询结果。

| 字段 | 含义 |
|---|---|
| `codexAccountProfileId` | 所属 profile |
| `weeklyUsedPercent` | Codex 7d 窗口返回的已使用百分比，范围 `0...100` |
| `weeklyResetsAt` | 7d 窗口重置时间 |
| `observedAt` | daemon 从 rate-limit 事件观测到该值的时间 |
| `credentialVersion` | 产生该快照的会话启动时使用的凭据版本 |
| `sourceSessionId` | 对应 Paws 会话，仅供归属校验与审计，不返回前端 |

前端以 `100 - weeklyUsedPercent` 计算并取整显示剩余百分比。更新时，Server 必须验证 `sourceSessionId` 的审计快照与 profile、设备及 `credentialVersion` 匹配；只有 `observedAt` 更新的合法观测才能覆盖现有快照，防止任意机器的“最新 usage”串到其他账号。

展示规则：

- 有有效快照：`7d 剩余 68% · 周日 10:30 重置`；时间按用户本地时区格式化。
- 从未收到快照：`7d 额度未知`。
- `observedAt` 超过 24 小时：保留最后数值并追加“可能已过期”。
- `weeklyResetsAt` 已经过期：不沿用上一窗口百分比，显示 `7d 额度未知 · 等待下次 Paws 会话更新`。
- profile 失效：保留最后一次额度用于辨识，同时优先显示“账号已失效”，不得把失效误报为额度耗尽。

### 5.3 Machine Codex Binding
...
- 每个账号显示最近一次 `7d 剩余百分比 + 重置时间`；未知或超过 24 小时的数据按快照规则明确标记。
- 不提供“刷新额度”按钮，也不在后台轮询；用户正常使用该账号启动 Paws Codex 会话后自动更新。
...
8. Codex 返回新的 rate-limit 事件时，daemon 只提取 7d `usedPercent`、`resetsAt` 和观测时间，连同会话归属回写对应 profile 的额度快照。
...
- `PUT /v1/codex-accounts/:id/quota-snapshot`：daemon 按已启动会话归属回写最新 7d 额度快照。
...
| 尚无 7d 额度事件 | 显示“7d 额度未知”；不为探测额度启动 Codex 请求 |
| 额度快照超过 24 小时 | 保留最后数值并标记“可能已过期” |
| 额度窗口已经重置 | 不显示上一窗口剩余值，等待下一次 Paws 会话被动更新 |
...
- 额度回写只接受已归属 Paws 会话的 7d 数值与时间字段；不上传 prompt、response、Token 或完整 Codex 事件。
...
- Paws Codex 会话收到 7d rate-limit 事件后，只更新实际使用 profile 的额度卡片，并显示剩余百分比和本地化重置时间。
- 没有额度事件、快照超过 24 小时、窗口已重置和账号失效时分别显示已定义的未知、过期与失效状态。
- 打开设备环境、点击重新检测或等待后台轮询都不会为刷新额度启动 Codex 请求。
...
- App：账号库、复制指令、逐台绑定、离线设备、7d 额度新鲜度、失效状态和启动阻断测试。
- 集成：两 profile / 两设备启动、恢复、分叉、额度快照不串号，以及全局 `auth.json` 不变测试。
...
3. 逐台设备 Codex 默认账号行；
4. 未绑定和账号失效的失败关闭状态；
5. 账号卡片中的 7d 剩余额度、重置时间，以及未知和过期状态。

标注稿只使用公开、脱敏的示例数据，并提供正常、未绑定、额度过期和账号失效四种可切换状态。示例百分比与时间必须明确属于设计演示，不代表真实账号数据。
