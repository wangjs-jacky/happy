...
截至本文快照，App 与 Server 的 `@paws/plugins` 均固定在 commit `1c77d6aea345000c9b543ff34d680140f234d3c5`：
...
该 commit 与插件仓库当前 `main` 合并提交 `eb24f765b7cb43827701250bb9f1654897fb2c68` 的 Git tree 相同。固定完整 SHA 是正确的供应链基础，但插件仓库有新提交或新 Release 时，Paws 不会自动获得更新，必须显式更新依赖并重新构建 App 与 Server。
...
3. App 使用当前 Manifest 的精确 `version` 调用 `PUT /v1/plugins/:pluginId`；
4. Registry 查找可信 definition，校验版本并调用插件自己的 `normalizeConfiguration`；
5. 更新配置时，空的 secret 字段会沿用已有密钥；首次安装仍必须提供必填 secret；
6. Server 加密整个 `{ version, configuration }` 后写入账号安装记录；
7. App 刷新目录；只有“已安装且版本等于当前 Manifest”的 contribution 才会被 Client Host 解析；
8. 业务请求到来时，Server 再次检查安装、版本和 Manifest 权限，不依赖 UI 是否隐藏按钮。
...
NOT_INSTALLED
      │ PUT current version
      ▼
CURRENT ───── Manifest version changed ────► STALE
   │  ▲                                      │
   │  └──────── PUT current version ─────────┘
   │
...
- `CURRENT`：贡献点可见，Runtime 可以通过门禁；
- `STALE`：市场可以提示更新，贡献点和 Runtime 都 fail closed；
...
3. 插件必须已安装且版本匹配；
4. Adapter 要求的 permission 必须出现在 Manifest 声明中。
...
| 优先级 | 问题 | 实际影响 | 建议 |
|---|---|---|---|
| P0 | `paws-plugins` 的 `package.json` 已是 `0.2.0`，但审阅当日 GitHub 最新 Release 仍为 `v0.1.0` | Manifest v2 已进入 Paws，但对应 `0.2.0` 的 tgz、catalog、checksum 和 provenance 尚未形成正式发布闭环 | 发布不可移动的 `v0.2.0`，核对产物后让 Paws 固定到该 Release 对应完整 commit |
| P1 | “动态插件”容易被理解为动态下载代码 | 产品预期与真实安全边界不一致 | 对外统一称为“第一方受信插件 / 动态启用”；安装页明确写“启用当前版本已内置的插件” |
| P1 | 权限只有 Manifest 声明，没有账号级授权快照 | 安装即隐式接受所有声明权限，无法单独审阅、撤销或比较升级新增权限 | 安装前展示权限 diff；持久化用户 grant；Broker 同时检查声明、grant、安装和版本 |
| P1 | Runtime 每次分别调用多个 `requirePermission` 和 `requireConfiguration` | 一次请求会重复读取并解密同一安装记录；能力越多越容易遗漏某个门禁 | 提供一个更深的 `openPluginRuntime(accountId, pluginId, requirements)` Interface，一次返回经过校验的短生命周期 capability context |
| P1 | App 的多个 `usePlugins()` 消费者都会主动 refresh | 已有串行同步避免旧响应覆盖，但仍有重复请求；各 Hook 只有自己的 loading，错误被吞掉 | 增加账号级 Catalog Owner Module，统一暴露 `idle/loading/ready/error` snapshot，消费者只订阅 |
...
下一阶段应先加深现有 Interface，而不是立刻做一个复杂 Extension Host：

1. 完成 `v0.2.0` 发布闭环，并把“构建时集成、运行时启用”写进产品文案；
2. 合并权限、安装、版本和配置读取为一次 Runtime capability context；
3. 让插件目录成为账号级单一数据源，补齐错误和刷新状态；
4. 把 UI Slot 的 renderer 注册真正通用化；
5. 增加 permission grant、配置 migration 和安装审计；
6. 只有出现明确的第三方代码分发需求后，再建设签名 Artifact 与隔离运行时。
